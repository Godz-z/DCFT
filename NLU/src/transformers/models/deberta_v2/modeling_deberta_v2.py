...
class DebertaV2SelfOutput(nn.Module):
    def __init__(self, config, lora_C=None):
        super().__init__()
        if config.apply_lora and "attention.output" in config.lora_module:
            if config.lora_type == "frd":
                ...
                self.dense = lora.SVDLinear(config.hidden_size, config.hidden_size, r=config.lora_r, 
                                            lora_alpha=config.lora_alpha, merge_weights=False, lora_C=lora_C)
            else:
...
class DebertaV2Attention(nn.Module):
    def __init__(self, config, lora_C=None):
        super().__init__()
        self.self = DisentangledSelfAttention(config, lora_C=lora_C)
        self.output = DebertaV2SelfOutput(config, lora_C=lora_C)
        self.config = config
...
class DebertaV2Layer(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # self.lora_C = nn.Parameter(torch.zeros(1, 1, 8, 8), requires_grad=True)
        # nn.init.normal_(self.lora_C, mean=0.0, std=0.02)
        # self.attention = DebertaV2Attention(config, lora_C=self.lora_C)
        # self.intermediate = DebertaV2Intermediate(config, lora_C=self.lora_C)
        # self.output = DebertaV2Output(config, lora_C=self.lora_C)
...
class DebertaV2Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.layer = nn.ModuleList([DebertaV2Layer(config, lora_C=lora_C) for _ in range(config.num_hidden_layers)])
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    def __init__(self, config):
        ...
        # self.encoder = DebertaV2Encoder(config, lora_C= self.lora_C)
